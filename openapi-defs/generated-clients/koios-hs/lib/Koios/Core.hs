...
module Koios.Core where

import Koios.MimeTypes
import Koios.Logging
...
-- | display the config
instance P.Show KoiosConfig where
  show c =
    T.printf
...
-- | constructs a default KoiosConfig
--
-- configHost:
--
...
-- @"koios/0.1.0.0"@
--
newConfig :: IO KoiosConfig
newConfig = do
    logCxt <- initLogContext
    return $ KoiosConfig
        { configHost = "https://api.koios.rest/api/v0"
        , configUserAgent = "koios/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => KoiosConfig -> auth -> KoiosConfig
addAuthMethod config@KoiosConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: KoiosConfig -> IO KoiosConfig
...
withStderrLogging :: KoiosConfig -> IO KoiosConfig
...
withNoLogging :: KoiosConfig -> KoiosConfig
...
data KoiosRequest req contentType res accept = KoiosRequest
  { rMethod  :: NH.Method   -- ^ Method of KoiosRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of KoiosRequest
  , rParams   :: Params -- ^ params of KoiosRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (KoiosRequest req contentType res accept) NH.Method
rMethodL f KoiosRequest{..} = (\rMethod -> KoiosRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (KoiosRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f KoiosRequest{..} = (\rUrlPath -> KoiosRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (KoiosRequest req contentType res accept) Params
rParamsL f KoiosRequest{..} = (\rParams -> KoiosRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (KoiosRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f KoiosRequest{..} = (\rAuthTypes -> KoiosRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => KoiosRequest req contentType res accept -> param -> KoiosRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: KoiosRequest req contentType res accept -> param -> KoiosRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: KoiosRequest req contentType res accept -> param -> KoiosRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method
          ...
          -> KoiosRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = KoiosRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     KoiosRequest req contentType res accept
  -> [NH.Header]
  -> KoiosRequest req contentType res accept
...
removeHeader :: KoiosRequest req contentType res accept -> [NH.HeaderName] -> KoiosRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => KoiosRequest req contentType res accept -> KoiosRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => KoiosRequest req contentType res accept -> KoiosRequest req contentType res accept
...
addForm :: KoiosRequest req contentType res accept -> WH.Form -> KoiosRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: KoiosRequest req contentType res accept -> NH.Part -> KoiosRequest req contentType res accept
...
_setBodyBS :: KoiosRequest req contentType res accept -> B.ByteString -> KoiosRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: KoiosRequest req contentType res accept -> BL.ByteString -> KoiosRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => KoiosRequest req contentType res accept -> P.Proxy authMethod -> KoiosRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> KoiosRequest req contentType res accept
    -> IO (KoiosRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> KoiosConfig
  -> IO (KoiosRequest req contentType res accept)
_applyAuthMethods req config@(KoiosConfig {configAuthMethods = as}) =
  foldlM go req as
  where
